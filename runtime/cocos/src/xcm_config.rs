...
use regionx_runtime_common::assets::{RX_ASSET_ID, RELAY_CHAIN_ASSET_ID};
...
impl Convert<AssetId, Option<MultiLocation>> for AssetIdConverter {
	fn convert(id: AssetId) -> Option<MultiLocation> {
		match id {
			...
			RX_ASSET_ID => Some(MultiLocation::here()),
			_ => None,
		}
	}
}
...
impl Convert<MultiLocation, Option<AssetId>> for AssetIdConverter {
	fn convert(location: MultiLocation) -> Option<AssetId> {
		match location {
			...
			MultiLocation { parents: 0, interior: Here } => Some(RX_ASSET_ID),
			_ => None,
		}
	}
}
