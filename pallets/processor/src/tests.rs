...
fn fulfill_order_works() {
	new_test_ext(vec![(2000, 1000), (10, 1000), (11, 1000), (12, 1000)]).execute_with(|| {
		...
		assert_ok!(Regions::set_record(region_id, RegionRecord { end: 8, owner: Some(1), paid: None }));
		assert_noop!(
...
fn cannot_fulfill_expired_order() {
	new_test_ext(vec![]).execute_with(|| {
		...
		let region_id = RegionId { begin: 0, core: 0, mask: CoreMask::complete() };
		assert_ok!(Regions::mint_into(&region_id.into(), &region_owner));
		assert_ok!(Regions::set_record(region_id, RegionRecord { end: 0, owner: Some(1), paid: None }));
